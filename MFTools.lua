...
local function downloadDependencies()
    createDirectories()
    sampAddChatMessage("{3498DB}[MFTools] {FFFFFF}Íà÷àëàñü ïåðâè÷íàÿ óñòàíîâêà êîìïîíåíòîâ. Ïîæàëóéñòà, ïîäîæäèòå...", -1)
    
    for i, file in ipairs(files_to_download) do
        local dest = getWorkingDirectory() .. "\\" .. file.path
        if not doesFileExist(dest) then
            -- Äîáàâëåí îáõîä êýøà äëÿ ïåðâè÷íîé óñòàíîâêè
    ...
    sampAddChatMessage("{88FF88}[MFTools] {FFFFFF}Óñòàíîâêà óñïåøíî çàâåðøåíà! Ñêðèïò ïåðåçàãðóæàåòñÿ...", -1)
    thisScript():reload()
end

local function checkForUpdates()
    local jsonPath = getWorkingDirectory() .. "\\mft_update.json"
    -- Îáõîä êýøà äëÿ JSON ðàáîòàåò îòëè÷íî
            ...
            if f then
                ...
                if success and data and tonumber(data.version) then
                    ...
                    if tonumber(data.version) > SCRIPT_VERSION then
                        ...
                        sampAddChatMessage("{3498DB}[MFTools] {FFFFFF}Äîñòóïíî îáíîâëåíèå: {FFDD00}" .. updateVersionText, -1)
                        sampAddChatMessage("{3498DB}[MFTools] {FFFFFF}Íàïèøèòå êîìàíäó â ÷àò {FFDD00}/mft_up{FFFFFF}, ÷òîáû óñòàíîâèòü åãî.", -1)
                    else
                        sampAddChatMessage("{3498DB}[MFTools] {FFFFFF}Ïðîâåðêà îáíîâëåíèé ïðîøëà óñïåøíî, îáíîâëåíèé íå îáíàðóæåíî.", -1)
                    end
                else
                    sampAddChatMessage("{FF0000}[MFTools] Îøèáêà ÷òåíèÿ update.json. Ïðîâåðüòå ñèíòàêñèñ ôàéëà íà GitHub!", -1)
                end
            end
        end
    end)
...
function main()
    ...
    sampAddChatMessage("{3498DB}[MFTools v" .. SCRIPT_VERSION_TEXT .. "] {FFFFFF}Ñêðèïò óñïåøíî çàãðóæåí! Ìåíþ: {FFDD00}/mft", -1)
    ...
    sampRegisterChatCommand("mft_up", function() 
        if updateAvailable then
            sampAddChatMessage("{3498DB}[MFTools] {FFFFFF}Íà÷èíàåì çàãðóçêó îáíîâëåíèÿ. Ýòî çàéìåò ïàðó ñåêóíä...", -1)
            
            lua_thread.create(function()
                -- 1. Ñêà÷èâàåì ãëàâíûé ôàéë ñ ÎÁÕÎÄÎÌ ÊÝØÀ
                ...
                -- 2. Ñêà÷èâàåì âñå çàâèñèìûå ôàéëû ñ ÎÁÕÎÄÎÌ ÊÝØÀ
                ...
                -- 3. Æäåì çàâåðøåíèÿ çàãðóçîê
                while downloaded < totalFiles or not mainScriptDownloaded do wait(100) end
                
                -- 4. Ïåðåçàãðóçêà
                sampAddChatMessage("{88FF88}[MFTools] {FFFFFF}Âñå ôàéëû óñïåøíî îáíîâëåíû! Ñêðèïò ïåðåçàãðóæàåòñÿ...", -1)
                thisScript():reload()
            end)
        else
            sampAddChatMessage("{FF0000}[MFTools] {FFFFFF}Íåò äîñòóïíûõ îáíîâëåíèé äëÿ çàãðóçêè.", -1)
        end
    end)
