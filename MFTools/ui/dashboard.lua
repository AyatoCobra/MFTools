...
function dashboard.draw()
    ...
    if dashboard.anims.menuOpen > 0.01 then
        ...
        imgui.TextColored(accentColorVec, "MFTools")
        ...
        imgui.TextColored(imgui.ImVec4(0.6, 0.6, 0.6, 1.0), "v1.0 (beta test)")
    ...
    if MFT.state.isCalibratingScreen then
        ...
        imgui.TextColored(imgui.ImVec4(1,1,1,1), t)
    ...
    if dashboard.anims.f8Notif > 0.01 then
        ...
        imgui.TextColored(imgui.ImVec4(c_accent[1], c_accent[2], c_accent[3], 1.0), text1)
        if MFT.fonts.title then imgui.PopFont() end
        
        imgui.Spacing()
        
        imgui.SetCursorPosX((windowW - tSize2.x) / 2)
        imgui.TextColored(imgui.ImVec4(1.0, 1.0, 1.0, 1.0), text2)
    ...
    if dashboard.anims.seqOvl > 0.01 then
        ...
        imgui.TextColored(imgui.ImVec4(c_accent[1], c_accent[2], c_accent[3], 1.0), tStr)
        if MFT.fonts.title then imgui.PopFont() end
        ...
        imgui.TextColored(imgui.ImVec4(c_accent[1], c_accent[2], c_accent[3], 0.8), hkStr)
        if MFT.fonts.small then imgui.PopFont() end
        ...
        if seq and seq.activeBind then
            ...
            for i = math.max(1, step - 1), math.min(total, step + 3) do
                ...
                if i < step then
                    imgui.TextColored(imgui.ImVec4(0.5, 0.5, 0.5, 1.0), u8"[+] " .. u8(lineText))
                elseif i == step then
                    ...
                    imgui.TextColored(imgui.ImVec4(1.0, 1.0, 1.0, 1.0), fullText)
                else
                    imgui.TextColored(imgui.ImVec4(0.6, 0.6, 0.6, 1.0), u8"[ ] " .. u8(lineText))
                end
            end
        else
            imgui.Spacing(); imgui.Spacing()
            ...
            imgui.TextColored(imgui.ImVec4(0.6, 0.6, 0.6, 1.0), s1)
            imgui.SetCursorPosX((winW - imgui.CalcTextSize(s2).x) / 2)
            imgui.TextColored(imgui.ImVec4(0.5, 0.5, 0.5, 1.0), s2)
            if MFT.fonts.main then imgui.PopFont() end
        end
        ...
        if MFT.state.isPlacingSeqOverlay then
            ...
            imgui.TextColored(imgui.ImVec4(0.2, 0.9, 0.2, 1.0), pt)
            if MFT.fonts.title then imgui.PopFont() end
        end
