...
-- Безопасный перенос текста для UTF-8 (mimgui)
...
-- Функция для получения имени бинда из базы по его ключу (напр. "1" или "2_4")
    ...
    if rSet.enabled then
        ...
        if rSet.menuMode == 0 then
            ...
            for i = 1, sectorsCount do
                ...
                -- ПОЛУЧЕНИЕ РЕАЛЬНОГО ИМЕНИ БИНДА (Режим одного кольца)
                local actualName = getBindNameByKey(rSet.binds[tostring(i)])
                local rawText = actualName or (u8"Бинд "..i)
            ...
            if activeGroupPreview ~= -1 then
                ...
                for j = 1, actionCount do
                    ...
                    -- ПОЛУЧЕНИЕ РЕАЛЬНОГО ИМЕНИ БИНДА (Внешнее кольцо)
                    local actualName = getBindNameByKey(grp.binds[tostring(j)])
                    local rawText = actualName or (u8"Бинд "..j)
                    local secText = wrapText(rawText, 24)
