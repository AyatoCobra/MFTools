...
-- ПОСТОЯННЫЕ БУФЕРЫ ДЛЯ РЕДАКТОРА БИНДОВ
local editSessionId = 0
local editBufName = ffi.new("char[256]")
local editBufText = ffi.new("char[8192]")
local editBufDelay = ffi.new("int[1]", 1000)
local editBufOvl = ffi.new("bool[1]", true)
local editBufHk = ffi.new("int[1]", 0)
...
function tabs.drawModalsAndOverlay(dash, dt, c_accent, sw, sh)
    if dash.anims.presetModalOpen > 0.01 and MFT.state.previewPresetId ~= -1 then
        ...
        if prData then
            ...
            for i, bind in ipairs(prData) do
                ...
                imgui.TextColored(c_accent, "%s", tostring(i) .. ". " .. (bind.name and u8(bind.name) or u8"Бинд"))
                if MFT.fonts.title then imgui.PopFont() end
                ...
                for j, line in ipairs(bind.lines) do
                    ...
                    imgui.TextColored(imgui.ImVec4(0.85, 0.85, 0.85, 1.0), "%s", u8(utils.sanitizePresetText(line)))
    ...
    -- ЗАГРУЗКА ДАННЫХ ПРИ НАЖАТИИ "ИЗМЕНИТЬ"
    if dash.forceLoadEditBind then
        local i = dash.forceLoadEditBind
        dash.forceLoadEditBind = nil
        MFT.state.editingModalIndex = i
        editSessionId = editSessionId + 1 -- Обновляем ID, чтобы ImGui перечитал буферы!
        ...
        local bind = currentList[i]
        if bind then
            editBufName[0] = 0
            ffi.copy(editBufName, u8(bind.name or ""))
            ...
            ffi.copy(editBufText, lC)
            
            editBufDelay[0] = bind.delay or 1000
            editBufOvl[0] = bind.showInOverlay == nil and true or bind.showInOverlay
            editBufHk[0] = bind.hotkey or 0
            dash.anims.capturingHotkeyModal = false
        end
    end

    -- РЕНДЕР ОКНА РЕДАКТИРОВАНИЯ
    if dash.anims.editModalOpen > 0.01 and MFT.state.editingModalIndex ~= -1 then
        ...
        imgui.InputText("##em_Name_"..editSessionId, editBufName, 256)
        imgui.PopItemWidth()
        
        imgui.NextColumn()
        imgui.TextUnformatted(u8"Кнопка активации:")
        imgui.PushStyleVarFloat(imgui.StyleVar.FrameBorderSize, 1.0)
        if dash.anims.capturingHotkeyModal then
            ...
            for k = 8, 255 do
                if imgui.IsKeyPressed(k) then
                    if k == vk.VK_BACK or k == vk.VK_ESCAPE then editBufHk[0] = 0
                    else editBufHk[0] = k end
                    dash.anims.capturingHotkeyModal = false
                    break
                end
            end
        else
            local kn = (editBufHk[0] == 0) and u8"Не назначена" or vk.id_to_name(editBufHk[0])
            if imgui.Button(kn.."##em_hk", imgui.ImVec2(-1, 30)) then dash.anims.capturingHotkeyModal = true end
            if editBufHk[0] ~= 0 then
                if MFT.fonts.small then imgui.PushFont(MFT.fonts.small) end
                imgui.TextColored(imgui.ImVec4(0.5, 0.5, 0.5, 1.0), "%s", u8"(Для удаления нажмите Backspace)")
        ...
        imgui.TextUnformatted(u8"Текст (Enter = новая строка):")
        imgui.InputTextMultiline("##em_Text_"..editSessionId, editBufText, 8192, imgui.ImVec2(-1, 180))
        ...
        local tags = {
            {"{target}", u8"Ближайший игрок (ID)"}, {"{namet}", u8"Имя ближайшего игрока"},
            {"{radial}", u8"Вставит ID игрока (ПКМ)"}, {"{weapon}", u8"Название оружия"},
            {"{dialog}", u8"Запросить ввод"}, {"{myname}", u8"Ваше имя"},
            {"{myid}", u8"Ваш ID"}, {"{time}", u8"Время (ЧЧ:ММ)"},
            {"{date}", u8"Дата (ДД.ММ.ГГГГ)"}, {"{veh}", u8"Название авто"},
            {"{hp}", u8"Здоровье"}, {"{armour}", u8"Броня"}
        }
        ...
        for ti, tag in ipairs(tags) do
            if imgui.Button(tag[1] .. "##em_tag_"..ti, imgui.ImVec2(btn_w, 30)) then 
                local current = ffi.string(editBufText)
                if #current + #tag[1] < 8190 then
                    ffi.copy(editBufText, current .. tag[1])
                end
            end
            ...
            if ti < #tags and next_button_x2 < window_visible_x2 then imgui.SameLine() end
        end
        ...
        imgui.TextUnformatted(u8"Задержка между строками (мс):")
        imgui.PushItemWidth(-1)
        imgui.SliderInt("##em_Delay_"..editSessionId, editBufDelay, 100, 5000)
        imgui.PopItemWidth()
        
        imgui.NextColumn()
        imgui.SetCursorPosY(imgui.GetCursorPosY() + 20)
        imgui.Checkbox(u8"Видно в оверлее##em_"..editSessionId, editBufOvl)
        imgui.Columns(1)
        ...
        if imgui.Button(u8"Сохранить", imgui.ImVec2(btnW, 45)) then
            local nameStr, textStr = ffi.string(editBufName), ffi.string(editBufText)
            if #nameStr > 0 and #textStr > 0 then
                ...
                if targetBind then
                    ...
                    targetBind.delay = tonumber(editBufDelay[0])
                    targetBind.showInOverlay = (editBufOvl[0] == true)
                    targetBind.hotkey = tonumber(editBufHk[0])
                end
                
                engine.saveData()
                sampAddChatMessage("{88FF88}[MFTools] {FFFFFF}Изменения сохранены!", -1)
