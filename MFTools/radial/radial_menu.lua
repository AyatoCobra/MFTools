-- Файл: MFTools/radial/radial_menu.lua
...
function radial_menu.drawUI()
    ...
    if style == 0 then
        ...
        for i = 1, sectorsCount do
            ...
            local rawText = item and (item.name and u8(item.name) or u8"Бинд") or u8"Пусто"
        ...
        elseif distToCenter > midRadius and distToCenter <= outerRadius then
            -- Если мышка перескочила сразу во внешнее кольцо
        ...
        -- Отрисовка внутреннего кольца (группы)
        ...
        -- Отрисовка внешнего кольца (бинды выбранной группы)
