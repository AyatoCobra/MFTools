...
local function downloadDependencies()
    createDirectories()
    sampAddChatMessage(u8:decode(u8"{3498DB}[MFTools] {FFFFFF}Началась первичная установка компонентов. Пожалуйста, подождите..."), -1)
...
function main()
    ...
    sampRegisterChatCommand("mft_up", function() 
        if updateAvailable then
            sampAddChatMessage(u8:decode(u8"{3498DB}[MFTools] {FFFFFF}Начинаем загрузку обновления. Это займет пару секунд..."), -1)
