...
function tab_binds.draw(dash, dt, c_accent, sb_color, c_text, availWidth)
    ...
    if imgui.BeginPopupModal(u8"Создание папки", nil, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove) then
        imgui.Spacing()
        imgui.TextUnformatted(u8"Название новой папки:")
        imgui.Spacing()
    ...
    if #MFT.state.folderPath > 0 then
        ...
        for i, p in ipairs(b_parts) do
            if p.col then
                imgui.TextColored(p.col, "%s", p.text)
            else
        ...
        if viewMode == 0 then
            for i, bind in ipairs(currentList) do
                ...
                if imgui.BeginDragDropSource(imgui.DragDropFlags.SourceAllowNullID) then
                    ...
                    if MFT.fonts.title then imgui.PushFont(MFT.fonts.title) end
                    imgui.TextColored(c_accent, "%s", u8"Перемещение:")
                ...
                imgui.TextColored(c_accent, "%s", tostring(i) .. ".")
                
                imgui.SetCursorPos(imgui.ImVec2(85, 24))
                if isFolder then
                    imgui.TextColored(c_accent, "%s", displayName)
                else
                    imgui.TextUnformatted(displayName)
                end
                if MFT.fonts.title then imgui.PopFont() end
                ...
                if not isFolder then
                    ...
                    if imgui.BeginPopupContextItem("MoveContext_"..i) then
                        if #MFT.state.folderPath == 0 then
                            imgui.TextColored(c_accent, "%s", u8"Переместить в:")
                            imgui.Separator()
                ...
                if not isFolder then
                    ...
                    if imgui.BeginPopupModal("RenameFolder_"..i, nil, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar) then
                        imgui.TextColored(c_accent, "%s", u8"ИЗМЕНЕНИЕ НАЗВАНИЯ")
                        imgui.Separator(); imgui.Spacing()
                        imgui.TextUnformatted(u8"Новое имя папки:")
                        imgui.Spacing()
                        imgui.PushItemWidth(-1)
                ...
                if isFolder then
                    ...
                    if imgui.Checkbox(u8"В оверлее##cb_"..i, ovlBool) then bind.showInOverlay = (ovlBool[0] == true); engine.saveData() end
                end
                
                if isFolder then
                    imgui.SetCursorPos(imgui.ImVec2(xEdit - 150, 28))
                    imgui.TextColored(imgui.ImVec4(0.6, 0.6, 0.6, 1.0), "%s", u8("Биндов: " .. #(bind.items or {})))
                end
    ...
    if imgui.BeginPopupModal("DeleteConfirmPopup", nil, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar) then
        imgui.TextColored(c_accent, "%s", u8"ПОДТВЕРЖДЕНИЕ УДАЛЕНИЯ")
        imgui.Separator(); imgui.Spacing()
        ...
        if delItem then
            ...
            imgui.TextUnformatted(u8"Вы действительно хотите удалить:")
            imgui.TextColored(imgui.ImVec4(1,1,1,1), "%s", u8(dName) .. " ?")
        end
