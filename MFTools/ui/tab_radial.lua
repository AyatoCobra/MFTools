...
local function wrapText(str, lineLen)
    ...
    for _, word in ipairs(words) do
        if #(lines[currentLine] .. word) > lineLen and lines[currentLine] ~= "" then
            ...
            if currentLine > 2 then break end
...
local function getBindNameByKey(keyStr)
    ...
    local parts = {}
    for p in string.gmatch(keyStr, "[^_]+") do table.insert(parts, tonumber(p)) end
    local cur = MFT.binds
    local lastBind = nil
    for _, p in ipairs(parts) do
        if not cur[p] then return nil end
        lastBind = cur[p]
        cur = cur[p].items or {}
    end
    if lastBind and lastBind.name then
        local n = lastBind.name
        if #n > 24 then n = string.sub(n, 1, 22) .. ".." end
        return u8(n)
    end
    return nil
end
    ...
    if rSet.enabled then
        ...
        if rSet.menuMode == 0 then
            ...
            for i = 1, sectorsCount do
                ...
                -- ИСПОЛЬЗУЕМ КАСТОМНОЕ ИМЯ ИЛИ ПЕРВОЕ СЛОВО
                ...
                if customName and customName ~= "" then
                    ...
                    local rawText = actualName or (u8"Пусто")
        ...
        if rSet.menuMode == 0 then
            ...
            utils.BeginCard(dash, dt, "RadialSettingsSector", 180, u8"ПРИВЯЗКА: СЕКТОР " .. activeSec, c_accent, sb_color)
            ...
            -- ПРИВЯЗКА И АВТО-УСТАНОВКА ИМЕНИ
            if imgui.Combo("##bindSelect", comboIdx, comboItems, #bindNames) then
                if comboIdx[0] == 0 then 
                    ...
                    local bName = getBindNameByKey(rSet.binds[secKey]) or u8"Бинд"
            ...
            -- ПОЛЕ ДЛЯ КАСТОМНОГО НАЗВАНИЯ СЕКТОРА
            imgui.TextColored(c_accent, u8"Название сектора в меню:")
            ...
            if rState.selectedOuter == -1 then
                ...
                if imgui.Combo("##gbnd_"..actIdx, comboIdx, comboItems, #bindNames) then
                    if comboIdx[0] == 0 then 
                        ...
                        local bName = getBindNameByKey(grp.binds[tostring(actIdx)]) or u8"Бинд"
                        local cText = bName:gsub("{%x%x%x%x%x%x}", "")
